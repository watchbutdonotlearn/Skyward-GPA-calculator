...
window.onload = function () {
  ...
  document
    .getElementById("closeWeight")
    .addEventListener("click", getGPAValues);
  document.getElementById("closeNoSave").addEventListener("click", showhide);
  document
    .getElementById("closeNoSave")
    .addEventListener("click", returnWeights);
  ...
  document
    ...
    .addEventListener("click", getAlgorithm);
  document
    .getElementById("closeSettings")
  ...
  document
    .getElementById("closeSettingsNoSave")
    .addEventListener("click", showhide2);
  document
    .getElementById("closeSettingsNoSave")
    .addEventListener("click", returndivgrades);
  document
    .getElementById("closeSettingsNoSave")
    .addEventListener("click", returnAlgorithm);
  document
    .getElementById("clearWeightSelections")
    .addEventListener("click", clearWeightForms);
  ...
  function returnAlgorithm() {
    chrome.storage.local.get(["storedAlgorithm"], function (items) {
      ...
      if (algValue === 0) {
        ...
        console.log(
          "changing stored algorithm from blank to default value of 1",
        );
        algValueCorrected = 1;
      }
  ...
  function returndivgrades() {
    chrome.storage.local.get(["storedGradesDivNum"], function (items) {
      ...
      if (numValue === 0) {
        ...
        console.log(
          "changing stored grade div number from blank to default value of 1",
        );
        numValueCorrected = 1;
      }
      //change radio of settings to saved value
      ...
      const newItem1 = document.createElement("radio");
      newItem1.innerHTML =
        '<input type="radio" id="gradeDiv' +
        numValue +
        '" name="gradeDivNumber" value="' +
        numValue +
        '" checked="checked">';
      selectedItem.parentNode.replaceChild(newItem1, selectedItem);
    });
