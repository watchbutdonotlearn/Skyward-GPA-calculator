  ...
  if (data.skywardDarkTheme == true) {
    ...
    if (page != "seplog01.w") {
      ...
      for (child in children) {
        if (children[child].src)
          children[child].src = chrome.runtime.getURL("SkyLogoBlue.png");
      }
    }
...
function saveGPAtoGraph() {
  chrome.storage.local.get(["GPAGraphArray"], function (data) {
    ...
    if (graphHasSet == 0) {
      ...
      if (
        GPAGraphArray[GPAGraphArray.length - 1].weighted == finalWeightedNumber
      ) {
    ...
    if (graphHasSet == 0) {
      ...
      GPAGraphArray.push({
        unweighted: finalUnweightedNumber,
        weighted: finalWeightedNumber,
        timestamp: timestamp,
      });
...
function calculateGPA() {
  ...
  if (tempGrades.includes(-1)) {
    // it is the first semester or not all grades are in, this just removes all elements that show up as -1, which would mean that the grades arent in yet.
  ...
  for (let i = ungradedIndexNumber.length - 1; i >= 0; i--) {
    //go in reverse order here to not mess up item indexes https://stackoverflow.com/questions/9425009/remove-multiple-elements-from-array-in-javascript-jquery
  ...
  currentGPAJSON["Current Skyward Grades"].setNumberOfClasses =
    numberOfGrades.toString();
  currentGPAJSON["Current Skyward Grades"].inputtedGPAValues =
    weightArrayOriginal;
  ...
  let GPAstr =
    ...
    (Math.round(finalUnweightedNumber * 1000) / 1000).toString() +
    " || ";
  ...
  /*else if(finalerrormessage === 2){
    ...
    GPAstr +=
      "Weighted GPA: " +
      (Math.round(finalWeightedNumber * 1000) / 1000).toString() +
      "</h2>";
  }
